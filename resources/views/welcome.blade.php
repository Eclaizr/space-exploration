...
@extends('layout.loader') <!-- Lien vers le code du loader -->
...
<header class="menu-bar">
    ...
    @auth 
    <nav class="nav-links">
        <form action="{{route('auth.logout')}}" method="POST" onsubmit="showUniverseLoader()">
            @method("delete")
            @csrf
    ...
    <!-- Navigation -->
     @guest
    <nav class="nav-links">
        <a href="{{ route('auth.login') }}" class="nav-button active action-button" onclick="showUniverseLoader()">Sign In</a>
        <a href="{{ route('register') }}" class="nav-button action-button" onclick="showUniverseLoader()">Sign Up</a>
    </nav>
    @endguest
</header>
...
<div class="video-container">
    <video autoplay muted loop id="background-video">
        <source src="{{ asset('videos/stars.mp4') }}" type="video/mp4">
